...
def addDepartment():
    try:
        ...
        if request.method == "POST":
            ...
            active = 1 if req['deptActive'] == 'YES' else 0  # 1=true, 0=false
...
def apimanageDepartment():
    ...
    if action == 'update':
        ...
        id = myList[0]
        dept = myList[1]
        dateCreated = str(myList[2])
        active = myList[3]
        dateInActive = str(myList[4])
    ...
    for row in resultList:
        aList = (row[0], row[1], str(row[2]), row[3], str(row[4]))
...
def data(orderId=None, dt_order_received=None, dt_order_returned=None, quantity=None):
    ...
    try:
        ...
        if action == 'updateOrderDate':
            ...
            dt_order_received = str(myList[1])
            dt_order_returned = str(myList[2])
        ...
        for row in resultList:
            ...
            aList = (row[0], row[1], purchaserName, row[3], row[4], str(row[5]), row[6], row[7], row[8], row[9], row[10], row[11], row[12],
...
def apimanageSupplier():
    ...
    for row in resultList:
        aList = (row[0], row[1], row[2], row[3], str(row[4]))
        d1 = dict(enumerate(aList))
        myList.append(d1)
...
def apimanagePurchaseOrderTable():
    ...
    for row in resultList:
        aList = (row[0], str(row[1]), str(row[2]), row[3], str(row[4]), row[5], row[6], row[7])
        # d1 = dict(enumerate(aList))
        myList.append(aList)
...
def apimanagePurchaser():
    ...
    if action == 'update':
        ...
        purchaserDateInActive = str(myList[4])
        purchaserDateCreated = str(myList[5])
    ...
    for row in resultList:
        aList = (row[0], row[1], row[2], row[3], str(row[4]), str(row[5]))
        #d1 = dict(enumerate(aList))
        #myList.append(d1)
        myList.append(aList)
...
def apimanageProvincialTaxRates():
    try:
        ...
        for row in resultList:
            aList = (row[0], row[1], f'{row[2]:.3f}', row[3], row[4])
            #d1 = dict(enumerate(aList))
            #myList.append(d1)
            myList.append(aList)
...
def apimanageUser():
    ...
    if action == 'update':
        ...
        username = myList[3]
        password = myList[4]
        createDate = str(myList[5])
        active = myList[6]
        dateInactive = str(myList[7])
    ...
    for row in resultList:
        aList = (row[0], row[1], row[2], row[3], row[4], str(row[5]), row[6], str(row[7]), row[8])
        #d1 = dict(enumerate(aList))
        myList.append(aList)
