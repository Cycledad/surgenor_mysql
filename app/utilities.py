...
def getConnection() -> mysql.connector.Connect:
    ...
    try:
        ...
        conn = mysql.connector.connect(host='localhost',
                                             ...
                                             user='root',
                                             password='MYSQLstrongPASSWORD')
        return conn

    except Exception as e:
        print(f'Could not establish a connection with mysql database {constants.DATABASE_NAME}, error {e}')
...
def getProvincialTaxRate(provincialCode: str) -> list:
    ...
    try:
        ...
        parm = (provincialCode,)
        stmt = """ select * from provincialTaxRates where provincialCode = %s """
...
def getUnitDesc(id: int) -> str:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (id,)
        stmt = """ select unitDesc from unit where id = %s """
        cur.execute(stmt, parm)
        row = cur.fetchone()
...
def getUnitId(desc: str) -> int:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (desc,)
        stmt = """ select Id from unit where unitDesc = %s """
        cur.execute(stmt, parm)
        row = cur.fetchone()
...
def getPartId(Desc: str) -> int:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (Desc,)
        stmt = """ select Id from Part where partNbr = %s and partInStock is True """
        cur.execute(stmt, parm)
        row = cur.fetchone()
...
def updatePurchaser(id: int, username: str, purchaserDeptId: int, purchaserActive: bool,
    ...
    try:
        ...
        stmt = """ update Purchaser set username = %s, purchaserDeptId = %s, purchaserActive = %s, purchaserDateInActive = %s, purchaserDateCreated = %s where id = %s """
...
def updateParts(id: int, partNbr: int, partDesc: str, partSupplierId: int, partQuantity: int, partInStock: bool,
    ...
    try:
        ...
        stmt = """ update Part set partNbr = %s, partDesc = %s, partSupplierId = %s, partQuantity = %s, partInStock = %s, partDateOutOfStock = %s, partDateCreated = %s where id = %s """
        cur.execute(stmt, parms)
        conn.commit()
...
def updateProvincialTaxRates(parms) -> None:
    ...
    try:
        conn = getConnection()
        cur = conn.cursor(prepared=True)
        stmt = """ update ProvincialTaxRates set provincialCode = %s, taxRate = %s, label = %s where id = %s """
        cur.execute(stmt, parms)
        conn.commit()
...
def updateSupplier(id: int, supplierName: str, supplierProv: str, supplierActive: bool, supplierDateCreated: str) -> None:
    ...
    try:
        ...
        stmt = """ update Supplier set supplierName = %s, supplierProv = %s, supplierActive = %s, supplierDateCreated = %s where id = %s """
        cur.execute(stmt, parms)
        conn.commit()
...
def updatePurchaseOrderTable(id: int, purchaseOrderDate: str, purchaseOrderReceivedDate: str, purchaseOrderActive: bool, purchaseOrderDateDeleted: str,
    ...
    try:
        ...
        stmt = """ update PurchaseOrder set purchaseOrderDate = %s, purchaseOrderReceivedDate = %s, purchaseOrderActive = %s, purchaseOrderDateDeleted = %s, purchaseOrderNbr = %s, purchaseOrderPurchaserId = %s, purchaseOrderPurchaserDeptId = %s where id = %s """
        cur.execute(stmt, parms)
        conn.commit()
...
def updateUser(id: int, givenName: str, surname: str, username: str, password: str, createDate: str, active: str,
    ...
    try:
        ...
        stmt = """ update User set givenName = %s, surname = %s, username = %s, password = %s, createDate = %s, active = %s, dateInactive = %s, securityLevel = %s where id = %s """
        cur.execute(stmt, parms)
        conn.commit()
...
def updateUserPassword(username: str, hashed_pw: str) -> bool:
    ...
    try:
        ...
        stmt = """ update User set password = %s where username = %s and active """
        cur.execute(stmt, parms)
        conn.commit()
...
def getSupplierName(id: int) -> str:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (id,)
        stmt = """ select supplierName from supplier where id = %s and supplierActive """
        # cur.execute('select unitDesc from unit where id = ?', parm)
        cur.execute(stmt, parm)
        row = cur.fetchone()
        # for row in cur:
...
def insertPurchaser(parms) -> None:
    ...
    try:
        ...
        stmt = """ INSERT INTO Purchaser (username, purchaserDeptId, purchaserActive) values (%s, %s, %s) """
        cur.execute(stmt, parms)
        conn.commit()
...
def getPurchaserId(username: str) -> int:
    ...
    try:
        ...
        stmt = """ select Id from purchaser where username = %s and purchaserActive """
        # cur.execute('select unitDesc from unit where id = ?', parm)
        cur.execute(stmt, parm)
        row = cur.fetchone()
...
def getPurchaserDeptId(username: str) -> int:
    ...
    try:
        ...
        parm = (username,)
        stmt = """ select purchaserDeptId from purchaser where username = %s and purchaserActive """
        # cur.execute('select unitDesc from unit where id = ?', parm)
        cur.execute(stmt, parm)
        row = cur.fetchone()
...
def getSupplierId(Name: str) -> int:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (Name,)
        stmt = """ select Id from supplier where supplierName = %s and supplierActive """
        # cur.execute('select unitDesc from unit where id = ?', parm)
        cur.execute(stmt, parm)
        row = cur.fetchone()
...
def getALLSupplierName() -> list:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        # parm=(Name,)
        stmt = """ select distinct supplierName from supplier where supplierActive """
        # cur.execute('select unitDesc from unit where id = ?', parm)
        cur.execute(stmt)
        row = cur.fetchall()
        for r in row:
...
def insertSupplier(parms) -> None:
    ...
    try:
        ...
        conn = getConnection()
        cur = conn.cursor(prepared=True)
        stmt = """ INSERT INTO supplier (supplierName, supplierProv, supplierActive) values (%s, %s, %s) """
        cur.execute(stmt, parms)
        conn.commit()
...
def insertDepartment(parms) -> None:
    ...
    try:
        ...
        conn = getConnection()
        cur = conn.cursor(prepared=True)
        stmt = """ INSERT INTO Department (deptName, active) values (%s, %s) """
        cur.execute(stmt, parms)
        conn.commit()
...
def deleteDepartment(id: int) -> None:
    ...
    try:
        ...
        parms = (dt, id,)
        stmt = """ update Department set active = False, dateInActive = %s where id = %s """
        cur.execute(stmt, parms)
        conn.commit()
...
def updateDepartment(id: int, dept: str, dateCreated: str, active: bool, dateInActive: str) -> None:
    ...
    try:
        ...
        stmt = """ update Department set deptName = %s, dateCreated = %s, active = %s, dateInActive = %s where id = %s """
        cur.execute(stmt, parms)
        conn.commit()
...
def getDepartmentId(deptName: str) -> int:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (deptName,)
        stmt = """ select Id from department where deptName = %s and active is True """
        cur.execute(stmt, parm)
        id = cur.fetchone()
...
def getTableItemById(id: int, tableName: str, colName: str) -> str:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (id,)
        stmt = f""" select {colName} from {tableName} where id = %s """
        # cur.execute('select unitDesc from unit where id = ?', parm)
        cur.execute(stmt, parm)
        row = cur.fetchone()
...
def insertPart(parms) -> None:
    ...
    try:
        ...
        stmt = """ insert into Part(partNbr, partDesc, partSupplierId, partQuantity, partInStock, partDateCreated) values (%s, %s, %s, %s, %s, %s) """
        cur.execute(stmt, parms)
        conn.commit()
...
def getPurchaser(username: str) -> str:
        ...
        try:
            ...
            cur = conn.cursor(prepared=True)
            parm = (username,)
            stmt = """ select u.givenName, u.surname from user u, purchaser p where p.purchaserActive and u.active and u.username = p.username and u.username = %s """
            cur.execute(stmt, parm)
            row = cur.fetchone()
            if row is None:
...
def getOrderNbr() -> int:
    ...
    try:
        ...
        if row is None:
            stmt = f""" insert into OrderNbr values 999 """
            cur.execute(stmt)
            row = 999
        else:
...
def updateOrderNbr(orderNbr: int) -> None:
    ...
    try:
        ...
        stmt = f""" update OrderNbrTbl set orderNbr = %s """
        cur.execute(stmt, parm)
        conn.commit()
...
def insertOrder(parms) -> None:
    ...
    try:
        ...
        stmt = """ INSERT INTO OrderTbl(orderNbr, orderSupplierId, deptName, orderPartDesc, orderQuantity, orderPartPrice, PO, orderUsername) values (%s,%s,%s,%s,%s,%s,%s,%s) """
...
def insertPurchaseOrder(purchaseOrderNbr: int, purchaserId: int, purchaserDept: int) -> None:
    ...
    try:
        ...
        stmt = """ INSERT INTO PurchaseOrder(PurchaseOrderReceivedDate, purchaseOrderNbr, purchaseOrderpurchaserId, purchaseOrderPurchaserDeptId) values (%s, %s, %s, %s) """
        cur.execute(stmt, parms)
        conn.commit()
...
def deletePurchaseOrder(orderId: int):
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm1 = (orderId,)
        stmt1 = """ select o.OrderNbr from orderTbl o where o.id = %s """
        cur.execute(stmt1, parm1)
        orderNbr = cur.fetchone()
        orderNbr = orderNbr[0]

        parm2 = (orderNbr,)
        stmt2 = """ select count(*) from orderTbl o where o.orderNbr = %s """
        ...
        stmt3 = """ delete from orderTbl where orderTbl.id = %s """
        cur.execute(stmt3, parm1)
        cur.fetchone()

        if nbrOfOrders == 1:
            stmt4 = """ delete from purchaseOrder where purchaseOrder.purchaserOrderNbr = %s """
...
def updateOrderReceivedDate(id: int, dt_order_received: str, dt_order_returned: str) -> None:
    ...
    try:
        ...
        stmt2 = """ select o.OrderNbr from orderTbl o where o.id = %s"""
        cur.execute(stmt2, parm1)
        orderNbr = cur.fetchone()
        orderNbr = orderNbr[0]

        parm3 = (orderNbr,)
        stmt3 = """ select count(*) from orderTbl o where o.orderNbr = %s and orderActive"""
        cur.execute(stmt3, parm3)
        nbrOfOrders = cur.fetchone()
        nbrOfOrders = nbrOfOrders[0]

        stmt4 = """ select count(*) from purchaseOrder where purchaseOrderNbr = %s and purchaseOrderActive"""
...
def updateOrderQuantity(id: int, quantity: int, colName: str) -> None:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (quantity, id,)
        stmt = f""" update orderTbl set {colName} = %s where id = %s """
        cur.execute(stmt, parm)
        conn.commit()
...
def updateOrderReceivedBy(parms) -> None:
    ...
    try:
        ...
        conn = getConnection()
        cur = conn.cursor(prepared=True)
        stmt = """ update orderTbl set orderReceivedBy = %s where id = %s """
        cur.execute(stmt, parms)
        conn.commit()
...
def updateActiveFlg(parms) -> None:
    ...
    try:
        ...
        stmt = """ update orderTbl set orderActive = %s where id = %s """
        cur.execute(stmt, parms)
        conn.commit()
        ...
        parm2 = (orderNbr,)
        stmt2 = """ select count(*) from orderTbl o where o.orderNbr = %s and o.orderActive """
        cur.execute(stmt2, parm2)
        nbrOfOrders = cur.fetchone()
        nbrOfOrders = nbrOfOrders[0]
        ...
        if nbrOfOrders == 0:
            stmt3 = """ update purchaseOrder set purchaseOrderActive = False where purchaseOrderNbr = %s """
            cur.execute(stmt3, parm2)
        else:
            stmt4 = """ update purchaseOrder set purchaseOrderActive = True where purchaseOrderNbr = %s """
...
def isPOArchived(PONbr: int) -> bool:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (PONbr, )
        stmt = """ select count(*) from ARCHIVE_OrderTbl where orderNbr = %s """
        cur.execute(stmt, parm)
        result = cur.fetchone()
        result = result[0]
...
def restoreOrderTbl(PONbr: int) -> bool:
    ...
    try:
        ...
        stmt = """ INSERT INTO OrderTbl (OrderNbr, OrderSupplierId, deptName, OrderPartDesc, OrderQuantity, OrderPartPrice, OrderReceivedDate, OrderReceivedBy, \
				...
				OrderReturnDate, OrderReturnQuantity, PO, OrderUsername, 1 FROM ARCHIVE_OrderTbl WHERE OrderActive = 0 and orderNbr = %s """
        cur.execute(stmt, parm)
        conn.commit()
...
def restorePurchaseOrderTbl(PONbr) -> bool:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (PONbr,)
        stmt = """ select count(*) from ARCHIVE_PurchaseOrder where purchaseOrderNbr = %s and purchaseOrderActive = False """
        cur.execute(stmt, parm)
        result = cur.fetchone()
        result = result[0]
        rtnCode = True
        if result > 0:  # ARCHIVED
            stmt = """ INSERT INTO PurchaseOrder (purchaseOrderDate, purchaseOrderReceivedDate, purchaseOrderActive, \
                    ...
                    FROM ARCHIVE_PurchaseOrder WHERE purchaseOrderActive = 0 and purchaseOrderNbr = %s """
            cur.execute(stmt, parm)
            conn.commit()
...
def deleteARCHIVEDOrder(PONbr):
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (PONbr,)
        stmt = """ delete from ARCHIVE_OrderTbl where OrderNbr = %s and orderActive = False """
        cur.execute(stmt, parm)
        conn.commit()
...
def deleteARCHIVEDPurchaseOrder(PONbr):
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (PONbr,)
        stmt = """ delete from ARCHIVE_PurchaseOrder where PurchaseOrderNbr = %s and purchaseOrderActive = False """
        cur.execute(stmt, parm)
        conn.commit()
...
def registerUser(username: str, hashed_pw: int, givenName: str, surname: str) -> None:
    ...
    try:
        ...
        stmt = """ insert into user (username, password, givenName, surname, securityLevel) values(%s, %s, %s, %s, %s) """
        cur.execute(stmt, parm)
        conn.commit()
        cur.close()
...
def getPassword(username: str) -> str:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (username,)
        stmt = """ select password from user where username = %s and active """
        cur.execute(stmt, parm)
        pw = cur.fetchone()
...
def getUserSecurityLevel(username: str) -> int:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (username,)
        stmt = """ select securityLevel from user where username = %s and active """
        cur.execute(stmt, parm)
        level = cur.fetchone()
        # conn.commit()
...
def getUser(username: str) -> bool:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (username,)
        #stmt = """ select username from user where username = %s and active """
        stmt = """ select * from user where username = %s and active """
        cur.execute(stmt, parm)
        result = cur.fetchone()
...
def getPurchaseOrderDate(purchaseOrderNbr: int) -> str:
    ...
    try:
        ...
        stmt = """ select purchaseOrderDate from PurchaseOrder where purchaseOrderNbr = %s and purchaseOrderActive """
        cur.execute(stmt, parm)
...
def getPurchaseOrderById(orderId: int) -> list:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (orderId,)
        stmt = """ select * from PurchaseOrder where id = %s and purchaseOrderActive """
        cur.execute(stmt, parm)
        row = cur.fetchone()
        # conn.commit()
...
def getOrderByOrderNbr(orderNbr: int, securityLevel: int) -> list:
    ...
    try:
        ...
        if securityLevel == constants.GOD_LEVEL:
            ...
            stmt = "select * from OrderTbl where orderNbr = ? and orderActive order by orderSupplierId asc"
        cur.execute(stmt, parm)
        rows = cur.fetchall()
...
def getUserLanguage(username: str) -> str:
    try:
        ...
        cur = conn.cursor(prepared=True)
        parm = (username,)
        stmt = "select language from user where username = %s and active is True"
        cur.execute(stmt, parm)
        lang = cur.fetchone()
...
def getActive(tableName: str, colName: str, btrue: bool) -> list:
    ...
    try:
        ...
        cur = conn.cursor(prepared=True)
        parms = (btrue, )
        stmt = f"select * from {tableName} where {colName} is %s"
        cur.execute(stmt, parms)
